...
version: 3.10.0-consolidated
status: GPT-5.5-optimized AIPSL runtime with 34 skills, 4 chains, HTML motion, PowerPoint reconstruction, knowledge graphs, anti-slop editing, and dataset-to-HTML dashboards
source: AIPSL v3.9.1 consolidated runtime plus dataset-to-HTML dashboard generation
...
Single-file AIPSL runtime with 34 skills, four workflow chains, standing analytical standards, the ODNI/ICD-203 probability/confidence lexicon, Structured Analytic Technique worksheets, and reusable patterns. Version 3.10.0 retains the GPT-5.5 profile: outcome-first instructions, selective loading, safe in-scope autonomy, and validation before delivery.

## Reading and Loading Rule

Read the Runtime Contract first. Activate only the selected skill, required pattern, and applicable governance. Do not treat the entire file as one simultaneous checklist. The canonical menu below is authoritative; later pass sections must not override it.
...
**Writing integrity:**
33. No-AI-Slop Editing & Detection

**Data visualization:**
34. Dataset-to-HTML Dashboard Generation
...
Use a chain only when its sequence is required.
...
| Edit or detect named AI-slop patterns while preserving the writer’s voice | 33 (use 11 for broader professional rewriting) |
| Turn an uploaded dataset into a contextual interactive HTML dashboard | 34 (use 12 for ingestion, 14 for analysis, and 15 for offline packaging as needed) |
...
3. Every material node attribute and link must carry provenance to a source location when available: file/document ID plus page, section, heading, line range, table/figure, or `user_context` reference.
4. Mark graph assertions as `explicit`, `derived`, or `unresolved`. A derived link requires a short stated basis; unsupported relationships are prohibited.
...
6. Do not merge entities solely because names resemble each other. Preserve aliases; merge only when identity is sufficiently supported. Ambiguous identities remain separate and may receive a `possible_same_as` link.
...
### Canonical Graph JSON Schema

Export graph JSON with this top-level shape only:

```json
{"nodes":[...],"links":[...]}
```

- **Nodes:** `id` is required. Optional loader fields: `label`, `group`, `type`, `size`, `color`, `x`, `y`, `z`. Source-grounded metadata/provenance may also be attached.
- **Links:** export `source`/`target`. On input also accept `from`/`to` or `subject`/`object`, then normalize. Optional relationship metadata/provenance may be attached.
- **Container aliases:** accept `entities`/`relationships` on input, normalize them to `nodes`/`links`, and always export `nodes`/`links`.
- **Metadata:** keep graph version, scope, source registry, conflict ledger, and graph notes in surrounding `Knowledge Graph Annex` Markdown, not as extra top-level JSON keys. Object-specific provenance stays on its node or link.

Example:

```json
{"nodes":[{"id":"N1","label":"System A","type":"System"},{"id":"N2","label":"Asset B","group":"asset"}],"links":[{"source":"N1","target":"N2","type":"depends_on"}]}
```

IDs remain stable within a revision. Use concise relationship names and domain-specific node types; preserve source wording when needed.
...
3. **Extract:** identify candidate entities, aliases, attributes, relationships, temporal qualifiers, source locations, and any loader aliases that require normalization.
4. **Resolve:** canonicalize labels; deduplicate only supported identities; preserve ambiguity and conflicting claims.
5. **Link:** create links only where evidence supports the relation; export `source`/`target`; label derived links and their basis.
6. **Validate:** check referential integrity, duplicate node IDs, optional duplicate link IDs, orphan links, unsupported claims, unresolved aliases, temporal inconsistencies, provenance coverage, and canonical `nodes`/`links` export shape.
7. **Export:** produce a `skill.md` containing operating instructions plus a `Knowledge Graph Annex` with the full canonical JSON graph.
...
Before delivery confirm: JSON parses; node IDs are unique; optional link IDs, when present, are unique; every link endpoint exists; every material assertion has provenance; no ambiguous entity merge is presented as certain; explicit/derived/unresolved states are populated correctly; contradictions are retained; temporal qualifiers do not exceed source support; external model context is separately tagged; and the exported `skill.md` contains the complete graph plus instructions for using and updating it.

## Pass 20 Quality Gate

Skill 32 ships only when the graph is source-grounded, provenance-complete for material assertions, referentially valid, ambiguity-preserving, contradiction-aware, temporally faithful where relevant, and embedded in a usable `skill.md`. Invented entities/links, silent conflict resolution, unsupported entity merges, untagged model knowledge, broken graph references, or silent truncation fail the gate.

# Pass 21 Runtime Layer — No-AI-Slop Editing & Detection

## Skill 33 — No-AI-Slop Editing & Detection

**Mission.** Edit prose into clearer, sharper, more human writing while preserving the writer’s actual voice, or detect named AI-slop patterns without rewriting. Use the minimum effective edit; do not convert distinctive writing into uniformly polished generic prose.

**Activation.** Route here when the user asks to remove AI slop/AI-sounding prose, make writing sound more human while preserving voice, or audit/detect slop patterns. Use Skill 11 for broader professional rewriting where a strict anti-slop rubric is not the main objective.

### Two Modes

- **Edit (default):** return the full edited draft plus a short **What changed** section.
- **Detect:** identify each applicable pattern by name, quote only the necessary source line, and give a short fix. Do not rewrite the draft, assign an “AI score,” or claim/guess whether AI authored it.

If no draft/source is available, request it. Ask about audience/format or reader outcome only when material and not inferable.

### Editing Rules

1. Read the whole draft first; identify its core point and internal voice signals: vocabulary, cadence, bluntness, humor, uncertainty, digressions, and polish level.
2. Preserve meaning, facts, examples, opinions, and useful rough edges. Do not invent claims, evidence, statistics, quotes, or stronger certainty. AIPSL protected facts, citations, analytical judgments, probability terms, and caveats remain invariant unless the user explicitly changes them.
3. Make the minimum effective edit. Leave strong human sentences alone; keep fragments, spoken cadence, humor, profanity, asides, or rough structure when they carry recognizable voice and remain clear.
4. Lead with the point when setup adds nothing; keep setup that creates context, tension, or character. Prefer active subjects, direct verbs, and concrete supported detail over abstraction.
5. Apply the **portability test**: if a sentence could be moved unchanged to another person, organization, product, or country, cut it or replace it with subject-specific substance already supported by the draft.
6. Show rather than label importance. Remove prose that tells the reader a point is important, surprising, subtle, obvious, or transformative when the evidence can carry that judgment.
7. Do not rotate synonyms merely for variety. Repeat the clearest term when consistency improves meaning.
8. Do not over-compress. The goal is recognizable human voice with less slop, not maximum brevity.

### Pattern Registry

Detect and fix these source-derived patterns when present:

- **Binary contrast / negative listing:** “not X, but Y” or “not X; not Y; Z” used for drama; state the substantive point directly.
- **Throat-clearing:** generic setup before the real claim; cut it.
- **Faux-insight setup:** claims that “everyone misses” or “nobody tells you”; remove the self-flattering frame.
- **Colon reveal:** dramatic noun-phrase-plus-colon reveal; use a normal sentence unless the colon serves a real list/label/quote function.
- **Superficial analysis:** trailing explanatory `-ing` clauses that simulate analysis; replace with the actual mechanism/consequence or remove.
- **Importance puffery:** pivotal/testament/vital/significance language; state the concrete fact.
- **Interpretive metadiscourse:** “the key point,” “as you can see,” “this distinction matters,” redundant “in other words”; delete or replace with support.
- **Weasel attribution:** unnamed experts/studies/reports; name the source or flag the unsupported claim.
- **Fake-strong verbs:** inflated verbs where “is/has/does” or a concrete action is clearer.
- **Synonym cycling:** unnecessary renaming of the same actor/object.
- **Dramatic fragmentation:** stacked punchy fragments used as a rhythm trick.
- **Robotic rhythm:** repeated sentence shapes, identical paragraph structures, or mechanical symmetry.
- **Rhetorical setup:** “what if,” “think about it,” “plot twist,” or self-answered question gimmicks; state the point.
- **Fake-profound kicker:** metaphorical/aphoristic mic-drop ending; delete it and end on a concrete takeaway/action already supported.
- **Summary-recap ending:** conclusion that merely repeats the piece; end on the last useful point or next action.
- **Formatting slop:** decorative emoji headings, gratuitous mid-sentence bold, needless micro-headings, or bullet lists that read better as prose.
- **Em-dash overuse:** avoid as a default rhythm device; short copy normally needs none, longer prose only occasional use when clearer than punctuation alternatives.

### Word/Phrase Watchlist

Cut generic filler unless required by source/domain/voice: `delve`, `foster`, `leverage`, `utilize`, `facilitate`, `empower`, `streamline`, `robust`, `cutting-edge`, `paradigm shift`, `game changer`, `tapestry`, `realm`, `beacon`, `multifaceted`, `meticulous`, `intricate`, `paramount`, `transformative`, `elevate`, `embark`, `supercharge`, `harness`, `ever-evolving`; plus empty lead-ins such as “it’s worth noting,” “at the end of the day,” “in today’s world,” “the reality is,” “going forward,” and “let’s dive in.”

### Workflow and Evaluation Gate

1. Read the full source and identify the core point/voice internally.
2. For Detect mode, produce only the named findings with minimal quoted evidence and concise fixes.
3. For Edit mode, apply the minimum changes, then self-check the complete revision.
4. **Pass only if:** meaning/protected facts and distinctive voice remain; strong human lines were not normalized; cutting matches actual slop; portability/attribution/pattern checks pass; robotic symmetry is absent; and the result reads naturally aloud.
5. If any check fails, revise before delivery. Edit mode ends with the complete draft plus **What changed**. Detect mode never rewrites or scores authorship.

## Pass 21 Quality Gate

Skill 33 fails on invented content, voice erasure, needless over-compression, unresolved material slop patterns, fabricated attribution, changed analytic/legal meaning, AI-authorship guesses, or detection scores.

**Source attribution.** Adapted for AIPSL from `petergyang/no-ai-slop` (MIT License), `skills/no-ai-slop/SKILL.md` blob `3aac728aa053b5e0a156259212fdea5a27ca9b67` and `eval.md` blob `328d37973231114292178079ef1eed82c816e7e7`, inspected on 2026-08-15. Repository main commit at inspection: `d30eddb9e04562234f2070b5ee63ca4649d9a05e`.\n# Pass 22 Runtime Layer — Dataset-to-HTML Dashboard Generation\n\n## Skill 34 — Dataset-to-HTML Dashboard Generation\n\n**Mission.** Convert an uploaded structured dataset into a decision-useful interactive `.html` dashboard. Inspect and profile the data first; then resolve any material context questions before rendering. Never choose metrics or chart emphasis solely because columns exist.\n\n**Activation.** Route here when the user uploads CSV, TSV, JSON, XLSX-compatible tabular data, or another inspectable structured dataset and asks for a dashboard, visual analytics page, KPI view, interactive report, or HTML visualization. Use Skill 12 to establish workbook/table structure, Skill 14 for nontrivial analysis or derived metrics, and Skill 15 when the dashboard must be self-contained/offline.\n\n### Mandatory Context Gate — Before Rendering\n\n1. **Profile first, without rendering the dashboard:** identify tables/sheets, row and column counts, grain, field names/types, date coverage, candidate measures/dimensions, missingness, duplicates, obvious unit/currency fields, and material data-quality issues. Do not infer unseen data.\n2. **Ask one grouped set of only the questions the data and current request cannot answer.** Typical questions are: intended audience; decision or recurring question the dashboard should support; most important outcomes/KPIs; required comparisons, targets, benchmarks, time windows, segments or filters; whether geography matters; and any required visual/branding or offline/browser constraints.\n3. Do **not** ask questions whose answers are directly recoverable from the uploaded dataset. Do not repeat context already supplied by the user. If purpose, audience, KPIs, and required slices are already sufficiently clear, acknowledge that the context gate is satisfied and proceed without unnecessary questioning.\n4. **Hard gate:** do not generate/render the final `.html` dashboard until all mission-critical context gaps are resolved. If a noncritical preference remains unspecified, choose a conservative default and document it.\n\n### Dashboard Design Rules\n\n- Start from the user’s decision need, then select visuals. Prefer a small set of meaningful KPIs and charts over a dense chart wall.\n- Match chart to question: line for ordered/time trends; bar for categorical comparison/ranking; stacked bar for additive composition; scatter for numeric relationships; pie/donut only for a small, meaningful part-to-whole; table for exact records. Do not use decorative 3D charts.\n- Provide filters only when they change interpretation or support the stated workflow. Typical controls: date range, category/segment, geography, status, and reset. Filters must update every dependent KPI/chart consistently.\n- Show units, denominators, date ranges, and definitions needed to interpret metrics. Label derived metrics and preserve the formula/logic used.\n- Preserve source values. Never invent missing targets, benchmarks, categories, dates, coordinates, or chart data. If the user requests a metric the dataset cannot support, state the gap before rendering.\n- Surface material data-quality warnings in the dashboard when they can affect interpretation.\n\n### HTML Implementation Contract\n\nDefault to one complete portable `.html` file with inline CSS/JavaScript and no remote calls, external fonts, tracking, or analytics. Use system fonts and native HTML/SVG/Canvas/browser APIs; no Web Animations API. Embed only the data needed for the dashboard when practical. If the dataset is too large to embed safely or responsively, preserve the requested data fidelity and use an explicitly documented same-directory data file or a pre-aggregated representation only with the user’s approval; never silently sample or truncate.\n\nThe dashboard should include, as justified by context: title/scope; KPI strip; primary charts; filters; definitions/data notes; reset control; responsive layout; accessible labels/keyboard behavior; and optional filtered-data export when useful. Desktop is the default target unless the user specifies otherwise, but the layout must degrade cleanly on smaller screens.\n\n### Validation\n\nBefore delivery: parse/open the HTML; verify no unintended external dependencies; confirm dataset row/control totals or aggregates reconcile to the source; test every filter/reset/control; verify derived metrics against their formulas; check date parsing, units, missing values, zero/negative/extreme cases, and categorical labels; ensure no `NaN`, `Infinity`, `undefined`, broken SVG/canvas references, or console errors; confirm charts and KPIs respond to the same filter state; and check that titles, units, legends, tooltips, and warnings remain readable.\n\n## Pass 22 Quality Gate\n\nSkill 34 ships only when the context gate was satisfied before rendering, the dashboard is traceable to the uploaded data, calculations reconcile, visual choices support the stated decision/use case, controls work, limitations are visible when material, and the delivered HTML opens as a complete usable artifact. Rendering before necessary context is resolved, inventing metrics/data, silent sampling, inconsistent filtering, broken controls, or decorative visuals that obscure the decision need fail the gate.\n
...
Skill 11 is the general professional editor. Use it for humanize/rewrite/polish/shorten/clarify requests where the user wants broad editorial improvement rather than the named anti-slop audit in Skill 33.

**Workflow.** Identify audience, product type, desired length/tone, citation preference, and requested intensity (proofread; light; professional rewrite; substantive edit; publication-ready). Diagnose clarity, tone, repetition, structure, unsupported claims, and citation gaps; preserve meaning and protected facts; edit to the requested level; then run final QA. For long documents, map the document, edit section-by-section, and finish with a global consistency pass. For high-stakes material, preserve caveats, legal/technical terms, uncertainty, and confidence language.

**Citation rule.** Add citations/endnotes only when requested and supported by supplied/verified sources. Never invent bibliographic details, quotations, page numbers, URLs, statistics, or source support.

**Output.** Deliver the complete polished product in the requested format. Route to Skill 33 when the user specifically asks to remove AI-sounding/slop patterns, preserve a distinctive personal voice against over-polishing, or detect named slop patterns without rewriting.

**Source.** Humanize Written AI Products Skill v2.0, SHA-256 `efe3adbc37bca7bca6e508a4addca8bf088389ab18c5919e5c72d72e8abe867b`.
...
Skill 11 preserves the user’s meaning while improving readability, organization, tone, and professional presentation. Protected elements must not be silently changed: numbers, percentages, dates, dollar values, probabilities, risk scores, rankings, counts, sample sizes, model parameters, confidence intervals, equations, names, quotations, legal shall/may/must distinctions, caveats, confidence/probability statements, assumptions, limitations, and alternative explanations.

**QA.** Confirm audience/length/tone requirements; facts and protected elements preserved; robotic repetition reduced; flow improved; formatting clean; citations included only when requested and genuinely supported; and no fabricated facts, quotations, sources, statistics, URLs, page numbers, or authorship claims. Never inflate or weaken analytical judgments merely to improve prose. Skill 33 applies the stricter named-pattern anti-slop rubric.

**Source.** `humanize_writer_skill_v2_0.md`, SHA-256 `efe3adbc37bca7bca6e508a4addca8bf088389ab18c5919e5c72d72e8abe867b`.
...
# Pass 11 Knowledge Annex — Performance Documentation Skills (Compact Reference)

Skills 19 and 20 are governed by their runtime definitions above. Preserve evidence-bound accomplishment extraction, derived-number labeling, optional performance-plan mapping without forced alignment, SMART goal construction, actual rating-framework/weighting conventions, source traceability, and the advisory-not-official boundary. Do not invent accomplishments, metrics, priorities, rating thresholds, or plan linkages.
...
- Skill 33 is voice-preservation-bound: it cannot invent content, claim AI authorship, score “AI-ness,” flatten distinctive voice into generic polish, or change protected facts/judgments while removing slop.
- Skill 34 is data-and-context-bound: it profiles the uploaded dataset before rendering, asks only unresolved material context questions, cannot invent measures/targets/data, silently sample, or deliver inconsistent filters, and must reconcile displayed calculations to the source.

## Traceability

Maintain bidirectional links appropriate to the task: provision → requirement → impact → action → verification; objective → criterion → evidence → score → recommendation; objective → risk → control → residual risk → treatment → indicator; defect → reproduction → root cause → change → test → release; need → requirement → component/interface → acceptance test → release evidence; Reporter context → SQL identifier/parameter → alias → visual/format rule → importer validation. Image reference → detected object → measured geometry/style/text → JSON/VBA object ID → rendered-slide validation. Source/context → source ID → entity/assertion → node/link ID → provenance locator → graph validation → exported Knowledge Graph Annex. Draft → core point/voice signals → named slop pattern → minimum edit/fix → evaluation gate. Dataset → profile → context gate → KPI/visual plan → HTML object/filter state → source reconciliation → dashboard validation. No unsupported interpretation, score, rating, claimed fix, metric, or requirement may appear only in the final narrative.

## Version Note — 3.10.0-consolidated

Preserves the GPT-5.5 optimization, no-WAAPI HTML motion, image-to-PowerPoint reconstruction, native `{"nodes":[...],"links":[...]}` knowledge-graph export, and No-AI-Slop editing/detection. Adds Skill 34, Dataset-to-HTML Dashboard Generation, with mandatory dataset profiling before rendering, a grouped context-question gate, decision-driven KPI/chart selection, offline-first HTML, no silent sampling or invented metrics, synchronized filtering, source reconciliation, and artifact validation. The consolidated file remains under the 250,000-byte/character constraint.
